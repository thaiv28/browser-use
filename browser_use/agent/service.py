...
from browser_use.critic import CriticGPT, ConfidenceError
...
class Agent(Generic[Context]):
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            ...
            ### TODO: ask critic model for score. if low -> pause and wait for input
            critic_score = self.critic.score(input_messages, model_output.current_state.next_goal)
            logger.info(f"Critic Score: {critic_score}")
            if critic_score <= 0.5:
                input("Requires human intervention. Press enter once next step is complete...")
                raise ConfidenceError("Low confidence in action")
            ...
            logger.debug("Agent paused due to low confidence")
            self.state.last_result = [
                ActionResult(
                    error='Human intervention was required. State may be different.'
                )
            ]
            return
        except Exception as e:
